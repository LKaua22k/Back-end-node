...
{            ...note,
    ...
    async index(req,res){
        ...
        if(tags){
                ...
                notes = await knex('tags').select(["notes.id","notes.title","notes.user_id"]).where('notes.user_id', user_id).whereLike('notes.title', `%${title}%`).whereIn('name', filteredTags).innerJoin('notes' , 'notes.id', 'tags.note_id').groupBy("notes.id").orderBy("notes.title")
